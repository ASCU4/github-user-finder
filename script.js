...
async function searchUser(){
    ...
    try{
     ...
     //https://api.github.com/user/ASCU4
...
function displayRepos(repos){
    if(repos.length == 0){
    reposContainer.innerHTML = '<div class="no-repos">No repositoriesfound</div>'; 
    return;  
    }
    ...
    repos.forEach(repo => {
        ...
        const updatedAt = fromatDate(repo.updated_at);
 ...
 function displayUserData(user){
    ...
    bioElement.textContent = user.bio || "NO bio Found";
    ...
    if(user.blog){
        blogElement.textElement = user.blog;
    ...
    if(user.twitter_username){
        ...
        twitterElement.href =`https://${user.twitter_username}`;
    }else{
